...
{
    device->objects[PROPERTY_OBJECT_USER_OTA].properties[1].content.value[0] = slot;
}
...
static inline void _set_checksum(knx_device_t *device, uint32_t checksum)
{
    ...
    memcpy(device->objects[PROPERTY_OBJECT_USER_OTA].properties[2].content.value, &tmp, sizeof(be_uint32_t));
}

static inline void _set_state(knx_device_t *device, ota_state_t state)
{
    device->objects[PROPERTY_OBJECT_USER_OTA].properties[4].content.value[0] = (uint8_t) state;
}
