...
const knx_addr_t knx_addr_broadcast = { .u8 = KNX_ADDR_BROADCAST };
const knx_addr_t knx_addr_undefined = { .u8 = KNX_ADDR_UNDEFINED };
...
{
    return ((int)byteorder_ntohs(*a) - (int)byteorder_ntohs(*b));
}
...
knx_addr_individual_t *knx_addr_individual(knx_addr_individual_t *result, uint8_t area, uint8_t line, uint8_t device)
{
    ...
    *result = byteorder_htons(((area & 0xf) << 12) + ((line & 0xf) << 8) + device);
...
knx_addr_group_t *knx_addr_group(knx_addr_group_t *result, uint8_t main, uint8_t mid, uint8_t sub)
{
    ...
    *result = byteorder_htons(((main & 0x1f) << 11) + ((mid & 0x7) << 8) + sub);
...
knx_addr_group_t *knx_addr_group2(knx_addr_group_t *result, uint8_t main, uint16_t sub)
{
    ...
    *result = byteorder_htons(((main & 0x1f) << 11) + sub);
...
char *knx_addr_individual_to_str(char *result, const knx_addr_individual_t *addr, uint8_t len)
{
    ...
    uint16_t tmp = byteorder_ntohs(*addr);
