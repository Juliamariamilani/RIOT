...
static int cmd_clk(int argc, char **argv)
{
    ...
    uint32_t lfe_freq = CMU_ClockFreqGet(cmuClock_LFE);
    uint32_t hf_freq = CMU_ClockFreqGet(cmuClock_HF);

    printf("LFXO available on PA0 (%" PRIu32 " Hz). HFXO available on PA1 (%" PRIu32 " Hz).\n",
           lfe_freq, hf_freq);
...
static int cmd_save(int argc, char **argv)
{
    ...
    puts("Asserting the save pin.");
    puts("Use the 'knx' command to initialize the save pin.");

    test = 9;

    if (gpio_is_valid(ncn5120_params[0].save)) {
        printf("Save pin state is %d.\n", gpio_read(ncn5120_params[0].save));
    }
    else {
        puts("NCN5120 save pins is not configured (GPIO_UNDEF).");
    }
